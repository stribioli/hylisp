...
udp_register_terminate(void *data, union sockunion *ds)
{
	...
	if (sendtov(skt, (char *)rpk->buf, rpk->buf_len, 0, (struct sockaddr *)&(ds->sa), slen) == -1) {
		 cp_log(LLOG, "failed\n");
		 perror("sendtov()");
		 close(skt);
		 return (FALSE);
...
udp_reply_terminate(void *data)
{
	...
	if (socket) {
		if (sendtov(socket, (char *)rpk->buf, rpk->buf_len, 0, (struct sockaddr *)&(local.sa), slen) == -1) {
			cp_log(LLOG, "failed\n");
			perror("sendtov()");
...
udp_referral_terminate(void *data)
{
	...
	if (socket) {
		if (sendtov(socket, rpk->buf, rpk->buf_len, 0, (struct sockaddr *)&(local.sa), slen) == -1) {
			cp_log(LLOG, "failed\n");
			perror("sendtov()");
			_free_rpl_pool_place(rpk, _rm_rpl);
			return (FALSE);
		...
		const struct prefix *eid)
{
	...
	case AF_INET: 
		...
		ih->ip_sum 		  = htons(ip_checksum((uint16_t *)ih, (ih->ip_hl)*2));
		break;
	case AF_INET6:
...
udp_request_ddt_terminate(void *data, const union sockunion *server, char terminal)
{
	...
	*/
	...
	/*=============================*/
	if (sendtov(skt, rpk->buf, rpk->buf_len, 0, (struct sockaddr *)&(servaddr.sa),slen) < 0) {
			cp_log(LLOG, "failed\n");
			perror("sendtov()");
			_free_rpl_pool_place(rpk, _rm_rpl);
			close(skt);
...
_forward(void *data)
{
	...
	if (sendtov(s,(void *)ih, (ih->ip_len), 0, (struct sockaddr *)&sin, sizeof (struct sockaddr)) < 0) {
		perror("sendto");
		close(s);
		return (FALSE);
	}
...
_forward_to_etr(void *data, struct db_node *rn)
{
	...
	if (sendtov(skt,(void *)packet, pkt_len, 0, (struct sockaddr *)&sin.sa, sin_len) < 0) {
		perror("sendto");
		close(skt);
		return (-1);
	}
...
udp_init_socket()
{
	...
	if (_virtual) {
		...
		if (getsockname(skfd, (struct sockaddr *) &bound_addr, &bound_addr_len) == -1) {
			perror("bind");
			close(skfd);
			exit(0);
		}
		
		virtual_udp_port = ntohs(bound_addr.sin_port);
		
		register_virtual_plane();
	}
...
_register_notify(void *data, struct site_info *site )
{
	...
	if (sendtov(skt, (char *)buf, pklen, 0, (struct sockaddr *)&(ds.sa), slen) == -1) {
			cp_log(LDEBUG, "failed\n");
			perror("sendtov()");
			free(buf);
			return (-1);
...
send_mr_ddt(uint32_t idx)
{
	...
	if (mr_lookups[idx].active) {
		...
		if (sendtov(skt, (char *)buf, buf_len, 0, (struct sockaddr *)&(servaddr.sa), slen) == -1) {
			cp_log(LLOG, "failed\n");
			perror("sendtov()");
			mr_lookups[idx].count++;
			return (FALSE);
